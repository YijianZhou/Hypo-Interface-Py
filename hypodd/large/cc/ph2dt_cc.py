...
def calc_dt(event_list, sta_dict, out_dt):
    ...
    num_pairs = int(num_events * (num_events-1) / 2)
    print('%s event pairs in total'%num_pairs)
    # calc dt
    dt_dataset = Diff_TT(event_list, sta_dict)
...
class Diff_TT(Dataset):
  ...
  """
  def __init__(self, event_list, sta_dict):
  ...
  def __getitem__(self, index):
    ...
    if loc_dev>loc_dev_thres: return [data_idx, temp_idx], {}
    sta_list = [sta for sta in pha_dict_data.keys() if sta in pha_dict_temp.keys()]
    if len(sta_list)<num_sta_thres: return [data_idx, temp_idx], {}
    ...
    for sta in sta_list:
        ...
        if min(data_dist,temp_dist)>dist_thres: dt_p, cc_p = [None]*2
        # read data & temp
        data_paths, data_tp, data_ts = pha_dict_data[sta]
        temp_paths, temp_tp, temp_ts = pha_dict_temp[sta]
        data_all, _, data_tt = read_data_temp(data_paths, data_tp, data_ts, data_loc)
        _, temp_all, temp_tt = read_data_temp(temp_paths, temp_tp, temp_ts, temp_loc)
